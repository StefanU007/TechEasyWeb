...
<template>
  <div
    ...
    :style="`color: ${appStore.selectedColor.three}; font-family: ${appStore.fonts[appStore.selectedFont]}; background-image: url(/assets/images/templates/2.avif);`"
  >
    <!-- Header -->
    <div class="px-2 sm:px-4 py-4 flex flex-col sm:flex-row items-center justify-between bg-[#2F4F4F] z-20">
      <img
        v-if="appStore.questionare.logo"
        :src="appStore.questionare.logo"
        class="w-20 h-12 sm:w-28 sm:h-16 object-scale-down mb-2 sm:mb-0"
        alt="Logo"
      />
      <img
        v-else
    ...
    <div class="flex flex-col lg:flex-row px-2 sm:px-5 pt-10 mb-16 gap-8">
      ...
      <div class="flex-1 flex flex-col items-center">
        <div class="relative w-full max-w-xl mx-auto" @click="hideEditIcon('name')">
          <div v-if="editIconVisibleName" @click.stop="hideEditIcon('name')"
      ...
      <div class="w-full lg:w-1/2 flex flex-col items-center lg:items-end lg:ml-auto mt-10 lg:mt-0">
        ...
        <div class="z-10 flex flex-col relative w-full max-w-2xl" @click="hideEditIcon('desc')">
          <div v-if="editIconVisibleDesc" @click.stop="hideEditIcon('desc')"
            ...
            <div class="w-1 h-5 bg-gray-200 absolute top-full left-1/2 -translate-x-1/2 lg:left-auto lg:right-6 lg:translate-x-0 shadow-lg"></div>
          </div>
          <p
    ...
    <div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-1 px-2">
        <div v-for="(image, index) in images.slice(0, 4)" :key="`img-${index}`"
          ...
          <span v-if="editIconVisible[index]" class="absolute top-2 right-2 bg-white p-1 shadow cursor-pointer rounded-full">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-800" viewBox="0 0 20 20" fill="currentColor">
              <path d="M17.414 2.586a2 2 0 010 2.828l-9 9a2 2 0 01-.707.293l-3.5.875a1 1 0 01-1.211-1.212l.876-3.5a2 2 0 01.293-.707l9-9a2 2 0 012.828 0zM16 5l-1-1-9 9v1h1l9-9z"/>
            </svg>
          </span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-1 mt-1 px-2">
        <div v-for="(image, index) in images.slice(4, 8)" :key="`img-${index + 4}`"
             class="relative cursor-pointer h-48 sm:h-72 w-full"
             @click="hideEditIconAndTriggerFileInput(index + 4)">
          <img :src="image.src" class="object-cover h-full w-full rounded-lg" />
          <span v-if="editIconVisible[index + 4]" class="absolute top-2 right-2 bg-white p-1 shadow cursor-pointer rounded-full">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-800" viewBox="0 0 20 20" fill="currentColor">
              <path d="M17.414 2.586a2 2 0 010 2.828l-9 9a2 2 0 01-.707.293l-3.5.875a1 1 0 01-1.211-1.212l.876-3.5a2 2 0 01.293-.707l9-9a2 2 0 012.828 0zM16 5l-1-1-9 9v1h1l9-9z"/>
            </svg>
          </span>
    ...
    <section class="container mx-auto px-4 py-16">
      <div class="text-center mb-12">
        <p class="m-auto text-[#274EBA] border-b-2 w-max font-raleway text-xs tracking-widest"
           :style="`color: ${appStore.selectedColor.one};`">
          OUR PRICES
        </p>
        <h2 class="text-2xl sm:text-4xl md:text-5xl font-bold text-[#274EBA] mt-4 font-playfair"
            :style="`color: ${appStore.selectedColor.one};`">
          Our Best Products
        </h2>
      </div>
...
<div>
    ...
    <!-- Footer -->
    <div class="bg-[#2F4F4F] text-white flex flex-col justify-center items-center" id="product">
      ...
      <div class="flex space-x-4 mt-10">
        <a href="#" class="flex items-center justify-center h-12 w-12 rounded-full border-2 border-white text-white text-xl"><i class="fab fa-facebook"></i></a>
        <a href="#" class="flex items-center justify-center h-12 w-12 rounded-full border-2 border-white text-white text-xl"><i class="fab fa-twitter"></i></a>
        <a href="#" class="flex items-center justify-center h-12 w-12 rounded-full border-2 border-white text-white text-xl"><i class="fab fa-instagram"></i></a>
        <a href="#" class="flex items-center justify-center h-12 w-12 rounded-full border-2 border-white text-white text-xl"><i class="fab fa-linkedin-in"></i></a>
        <a href="#" class="flex items-center justify-center h-12 w-12 rounded-full border-2 border-white text-white text-xl"><i class="fab fa-whatsapp"></i></a>
      </div>
      <footer class="mt-auto py-4 text-xs">
        <p class="text-center font-raleway">2023 &copy; DESIGNED BY <span class="border-b border-red-500">TechEasy</span></p>
      </footer>
    </div>
  </div>
</template>
...
const images = ref([
    { src: "/assets/template-2/1.jpeg" },
    { src: "/assets/template-2/2.jpeg" },
    { src: "/assets/template-2/3.jpeg" },
    { src: "/assets/template-2/4.jpeg" },
    { src: "/assets/template-2/5.jpeg" },
    { src: "/assets/template-2/6.jpeg" },
    { src: "/assets/template-2/7.jpeg" },
    { src: "/assets/template-2/8.jpeg" }
]);
